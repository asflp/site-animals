...
$.get('http://localhost:5000/get-cookies', "json")
    .done(function (response) {
        ...
        dict.set("role", responseJson.User.Role);

        if(nickname === " "){
            $('#text_my_questions').css({
                'display': 'none'
            });
        } else if(responseJson.User.Role === "moderator") {
            textMyQuestions.textContent = "Необработанные вопросы";
        }
...
function showPost (element) {
    ...
    itemBottomComment2.textContent = element.Comments.length ?? 0;
...
$.get('http://localhost:5000/get-questions', "json")
    .done(function (response){
...
function clickEvent(event){
    switch (event.target.id){
        ...
        default:
            ...
            if(dict.get('role') === "moderator"){
                let outputData = {
                    Id: event.target.closest('.question_item').id,
                    Status: null
                };

                if(event.target.alt === "approve") {
                    outputData.Status = "Одобрено"
                } else if(event.target.alt === "reject") {
                    outputData.Status = "Отклонено"
                }

                $.post('http://localhost:5000/update-status-questions', JSON.stringify(outputData), "json")
                    .done(function (){
                        window.reload();
                    });
...
categories.forEach((item) => item.addEventListener('click', eventCategory))
...
function eventCategory (event) {
    ...
    $.ajax({
        type: "POST",
        url: 'http://localhost:5000/get-questions-by-category',
        contentType: "application/json; charset=utf-8",
        data: category,
        dataType: "json",
        success: function(response) {
            textHeader.textContent = category;
            allItems.innerHTML = '';
            const questions = response.Result;
            questions.forEach(item => showPost(item));
        },
...
let textAllQuestions = document.getElementById('text_all_questions');
function clickTextAllQuestions () {
    $.get('http://localhost:5000/get-questions', "json")
        .done(function (response){
            textHeader.textContent = textAllQuestions.textContent;
            allItems.innerHTML = '';
            const questions = response.Result;
            questions.forEach(item => showPost(item));
        });
...
$('#text_all_questions').css({
    'background-color': '#FFEEDB',
    'cursor': 'default'
});

textAllQuestions.onclick = function () {
    $('#text_all_questions').css({
        'background-color': '#FFEEDB',
        'cursor': 'default'
    });
    ...
    clickTextAllQuestions();
}

let textAddQuestion = document.getElementById('text_add_question');
textAddQuestion.onclick = function () {
    window.location = 'http://localhost:5000/addquestion';
}

let textMyQuestions = document.getElementById('text_my_questions');
textMyQuestions.onclick = function () {
    $('#text_my_questions').css({
        'background-color': '#FFEEDB',
        'cursor': 'default'
    });
    ...
    clickTextMyQuestions();
}

function clickTextMyQuestions () {
    if(dict.get('role') === "moderator"){
        $.get('http://localhost:5000/raw-questions', "json")
            .done(function (response){
                textHeader.textContent = "Необработанные вопросы";
                allItems.innerHTML = '';
                const questions = response.Result;
                questions.forEach(item => showRawPost(item));
            });
    } else {
        $.get('http://localhost:5000/my-questions', "json")
            .done(function (response){
                textHeader.textContent = "Мои вопросы";
                allItems.innerHTML = '';
                const questions = response.Result;
                questions.forEach(item => showMyPost(item));
            });
    }
}

function showMyPost (element) {
    let item = document.createElement("div");
    item.className = "question_item";
    item.id = element.Id;
    let itemHeader = document.createElement("div");
    ...
    let itemPersonCategory = document.createElement("button");
    itemPersonCategory.type = "button";
    itemPersonCategory.textContent = element.Status;
    if(element.Status === "В обработке"){
        itemPersonCategory.style.backgroundColor = "#c0bcbc";
    } else if(element.Status === "Одобрено"){
        itemPersonCategory.style.backgroundColor = "#84a979";
    } else if(element.Status === "Отклонено"){
        itemPersonCategory.style.backgroundColor = "#b25050";
    }
    itemPerson.innerHTML += imgPerson.outerHTML + itemPersonDiv.outerHTML + itemPersonCategory.outerHTML;
    let itemText = document.createElement("div");
    itemText.className = "question_item__text";
    itemText.textContent = element.Text;
    item.innerHTML += itemHeader.outerHTML + itemPerson.outerHTML + itemText.outerHTML;

    allItems.appendChild(item);
}

function showRawPost (element) {
    let item = document.createElement("div");
    item.className = "question_item";
    item.id = element.Id;
    let itemHeader = document.createElement("div");
    ...
    let itemBottom = document.createElement("div");
    itemBottom.className = "question_item__bottom";
    itemBottom.style.gap = "20px";
    let imgApprove = document.createElement("img");
    imgApprove.src = "/../images/forum/helpers/done.svg";
    imgApprove.alt = "approve";
    let imgReject = document.createElement("img");
    imgReject.src = "/../images/forum/helpers/not-done.svg";
    imgReject.alt = "reject";
    itemBottom.innerHTML += imgApprove.outerHTML + imgReject.outerHTML;
    item.innerHTML += itemHeader.outerHTML + itemPerson.outerHTML + itemText.outerHTML + itemBottom.outerHTML;
